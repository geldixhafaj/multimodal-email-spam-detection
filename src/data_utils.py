...
def save_csv(rows: Iterable[Iterable[Any]], headers: Iterable[str], path: Path | str) -> None:
    """
...
# ------------------------
# Tiny timing context mgr
# ------------------------
class Timer:
    """
    Context manager for quick wall-clock timing.
...
# -------------------------------------
# Common filenames used across notebooks
# -------------------------------------
FILENAMES: Dict[str, Path] = {
    # Baseline artefacts
    "baseline_json": OUTPUTS_BASELINE_DIR / "baseline_distilbert.json",

    # Features & embeddings metadata
    "features_json": OUTPUTS_FEATURES_DIR / "feature_extraction.json",
    "struct_names_json": BASE_DIR / "struct_features" / "struct_feature_names.json",
    # You can add embeddings metadata if you store it
    "embeddings_json": OUTPUTS_FEATURES_DIR / "embeddings_extract.json",

    # Multimodal artefacts
    "mlp_json": OUTPUTS_MULTIMODAL_DIR / "multimodal_mlp.json",
    "mlp_state_dict": OUTPUTS_MULTIMODAL_DIR / "multimodal_mlp_state_dict.pt",
    "mlp_history_csv": OUTPUTS_MULTIMODAL_DIR / "multimodal_training_history.csv",
    "mlp_results_json": OUTPUTS_MULTIMODAL_DIR / "multimodal_results.json",

    # XAI exports
    "xai_global_csv": OUTPUTS_XAI_DIR / "shap_global_grouped.csv",
    "xai_beeswarm_csv": OUTPUTS_XAI_DIR / "shap_structural_beeswarm.csv",
    "xai_local_tp_csv": OUTPUTS_XAI_DIR / "shap_local_TP.csv",
    "xai_local_err_csv": OUTPUTS_XAI_DIR / "shap_local_ERR.csv",

    # Tables workbook
    "tables_xlsx": OUTPUTS_TABLES_DIR / "chapter3_tables.xlsx",
}

# ------------------------------------
# Small health‑check for the project
# ------------------------------------
def project_healthcheck(logger: Optional[logging.Logger] = None) -> Dict[str, Any]:
    """
    Validate that key folders exist and return a brief diagnostic.
    """
    diag = {"paths": {}, "writable": {}, "python": sys.version.split()[0]}
    for k, p in PATHS.items():
        ...
        try:
            ...
            diag["writable"][k] = True
        except Exception:
            diag["writable"][k] = False

    if logger is None:
        logger = get_logger("healthcheck", log_to_file=False)
    logger.info("Project healthcheck: %s", diag)
    return diag
...
# -----------------------------
__all__ = [
    "__package_name__", "__version__", "__author__",
    "BASE_DIR", "PATHS", "FILENAMES",
    "init_dirs", "get_logger", "seed_everything", "get_device",
    "load_json", "save_json", "save_csv",
    "Timer", "project_healthcheck",
]
``
